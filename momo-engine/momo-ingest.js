/**
 * 檔案：momo-ingest.js
 * v1.1.0 | 2026-05-18 | 新增：verifyMomoOrder（供 express-server.js 即時發貨用）
...
// ── 即時訂單驗證（供 express-server 自助領取使用）─────────────────────────────

/**
 * 驗證指定 MOMO 訂單是否存在且尚未出貨，並回傳商品名稱與數量
 *
 * 查詢邏輯：
 *  1. 呼叫 unsendCompanyQuery 取得所有未出貨訂單
 *  2. 以「完整子訂單號」或「基號前綴」篩選（例如輸入 26051818060954
 *     可同時比對到 26051818060954-001-001-001 和 26051818060954-001-001-002）
 *  3. 命中筆數即為數量（qty），商品名稱取第一筆
 *
 * 注意：若訂單已被 GAS 自動發貨系統處理（MOMO 狀態已出貨），
 *  unsendCompanyQuery 不會再回傳此訂單。這種情況請由 Phase 2（Sheets 補查）處理。
 *
 * @param {string} orderNo  消費者輸入的 MOMO 訂單編號（基號或完整子訂單號）
 * @returns {Promise<{ valid: boolean, qty: number, goodsName: string, message?: string }>}
 */
async function verifyMomoOrder(orderNo) {
  if (!MOMO_ENTP_CODE || !MOMO_ENTP_ID || !MOMO_ENTP_PWD || !MOMO_OTP_BACK_NO) {
    throw new Error('MOMO 環境變數不完整，無法執行訂單驗證');
  }

  let encrypted;
  try {
    encrypted = await fetchMomoOrders();
  } catch (err) {
    throw new Error(`MOMO SCM API 呼叫失敗: ${err.message}`);
  }
  ...
  if (!encrypted) {
    return {
      ...
      qty:       0,
      goodsName: '',
      message:   '目前無待出貨訂單，或訂單已出貨/不存在',
    };
  }

  let orders;
  try {
    orders = decryptMomoData(encrypted, MOMO_ENTP_PWD);
  } catch (err) {
    throw new Error(`MOMO 訂單資料解密失敗: ${err.message}`);
  }

  // 前綴比對：支援基號（26051818060954）比對到子訂單（26051818060954-001-001-001）
  const matched = orders.filter(o => {
    const on = String(o.orderNo || o.company_orderNo || '').trim();
    return on === orderNo || on.startsWith(orderNo + '-');
  });
  ...
  if (matched.length === 0) {
    return {
      ...
      qty:       0,
      goodsName: '',
      message:   '查無此訂單（可能已由自動發貨處理，或訂單編號有誤）',
    };
  }

  const goodsName = String(
    matched[0].goodsName || matched[0].entpGoodsName || ''
  ).trim();

  console.log(`[momo-ingest] verifyMomoOrder: "${orderNo}" 命中 ${matched.length} 件, 品名="${goodsName}"`);
...
module.exports = { startIngestScheduler, runIngestCycle, decryptMomoData, verifyMomoOrder };
