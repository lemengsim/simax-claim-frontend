...
export default function ClaimPage() {
  ...
  // ── Step 1 PIN 驗證子狀態 ──
  const [verified,   setVerified]   = useState(false);   // 是否已通過 /api/verify
  const [qty,        setQty]        = useState(1);        // 訂單件數
  const [goodsName,  setGoodsName]  = useState('');       // 商品名稱（提示用）
  const [pins,       setPins]       = useState(['']);     // 客戶輸入的 12 碼 PIN 陣列
  ...
  // ── Step 1b：送出 PIN → 正式發貨 ───────────────────────────────────
  const handleClaimWithPins = async (e) => {
    e.preventDefault();
    setLoading(true);
    setError('');
    ...
    <>
      ...
      <div className="page">
        <div className="card">
          ...
          {/* ════════════════ STEP 1b：輸入電子票務序號（PIN） ═══════════ */}
          {!specialStatus && step === 1 && verified && (
            <form className="form" onSubmit={handleClaimWithPins} autoComplete="off">

              {goodsName && (
                <div style={{ marginBottom: 16, padding: '10px 14px', background: 'rgba(99,102,241,0.06)', borderRadius: 10, fontSize: 13, color: 'var(--text)' }}>
                  📦 {goodsName}（共 {qty} 件）
                </div>
              )}
                ...
                <div className="field" key={i}>
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      setPins(prev => prev.map((p, idx) => idx === i ? val : p));
                      setError('');
                    }}
                  />
              ...
              <button type="submit" className="btn-submit" disabled={!canClaimWithPins}>
                {loading
              ...
              <button
                ...
                onClick={() => { setVerified(false); setPins(['']); setError(''); }}
              >
                ← 重新輸入訂單
              </button>
